...
export function generateStaticParams() {
  return countries
    .filter(c => c.type === 'residence' || c.type === 'both')
    .map((country) => ({
      slug: country.slug,
    }));
}
...
export async function generateMetadata({ params }: Props) {
  ...
  if (!country) {
    return {
      ...
      description: 'The requested residence page could not be found.',
    };
  }

  return {
    title: `${country.name} Residence by Investment | Passport Legend`,
    description: `Learn about ${country.name} residence by investment program. Benefits, requirements, process, and investment options.`,
  };
}

export default async function ResidenceByInvestmentPage({ params }: Props) {
  const { slug } = await params;
  const country = getCountryBySlug(slug);

  if (!country || (country.type !== 'residence' && country.type !== 'both')) {
    notFound();
  }
          ...
          <Container className="relative z-10">
            <div className="max-w-3xl animate-fade-in-up">
              <nav className="flex items-center gap-2 text-sm font-medium text-accent-400 mb-8 uppercase tracking-widest">
                ...
                <Link href="/residence" className="hover:text-white transition-colors">Residence</Link>
                <ChevronRight size={14} />
            ...
            <Container>
              <div className="text-center max-w-3xl mx-auto mb-20">
                ...
                <p className="text-lg text-neutral-600 dark:text-neutral-400 font-light">
                  Secure your future with premium residency status in one of the world's most desirable locations.
                </p>
              </div>
